...
function fixture(t,harness){
 ...
 const root=path.join(home,'source');fs.mkdirSync(path.join(root,'workspaces'),{recursive:true});
 fs.writeFileSync(path.join(root,'workspaces/test.yaml'),`instructions: Keep workspace context.\nconnections:\n  proof:\n    type: mcp\n    url: https://example.com/mcp\n    auth: native\n    description: Use the selected project.\n  local:\n    type: mcp\n    command: example-server\n    env_vars: [MY_SECRET]\n`);
...
for(const harness of ['claude','codex']){
 test(`${harness} global workspace loads idempotently and unloads only owned config/context`,t=>{
  const f=fixture(t,harness);const a=loadWorkspace(f.root,'test',f.options);
  ...
  assert.deepEqual(loadWorkspace(f.root,'test',f.options),a);
  ...
  fs.rmSync(f.root,{recursive:true});unloadWorkspace('test',f.options);
 ...
 test(`${harness} existing MCP collisions are refused without changes`,t=>{
  ...
  assert.throws(()=>loadWorkspace(f.root,'test',f.options),/will not be overwritten or adopted/);
  assert.equal(fs.readFileSync(f.config,'utf8'),existing);
  assert.equal(fs.readFileSync(f.instructions,'utf8'),'Existing user instructions.\n');
 });
 test(`${harness} unload preserves edited connections and instructions`,t=>{
  const f=fixture(t,harness);loadWorkspace(f.root,'test',f.options);
  ...
  assert.throws(()=>unloadWorkspace('test',f.options),/Managed connection changed/);
  ...
  assert.throws(()=>unloadWorkspace('test',f.options),/instructions changed/);
...
test('global workspace enforces one selection per harness and rolls back on receipt failure',t=>{
 ...
 try{assert.throws(()=>loadWorkspace(f.root,'test',f.options),/simulated/);}finally{fs.renameSync=originalRename;}
 assert.equal(fs.readFileSync(f.config,'utf8'),f.original);
 assert.equal(fs.readFileSync(f.instructions,'utf8'),'Existing user instructions.\n');
 loadWorkspace(f.root,'test',f.options);
 fs.copyFileSync(path.join(f.root,'workspaces/test.yaml'),path.join(f.root,'workspaces/other.yaml'));
 assert.throws(()=>loadWorkspace(f.root,'other',f.options),/Unload the current/);
});
test('native config overrides and user instruction override file are respected',t=>{
 ...
 const item=loadWorkspace(f.root,'test',{...f.options,env:{CODEX_HOME:'/generated',AGENT_FARM_NATIVE_CODEX_HOME:native}});
...
test('global workspace CLI load/list/unload operates on isolated user files',t=>{
 ...
 let result=run(['workspace','load','test','--harness','claude','--config-root',f.root]);assert.equal(result.status,0,result.stderr);
 ...
 result=run(['workspace','unload','test','--harness','claude']);assert.equal(result.status,0,result.stderr);
});
test('symlink native configuration is refused without replacing it',t=>{
 ...
 assert.throws(()=>loadWorkspace(f.root,'test',f.options),/symlink/);
