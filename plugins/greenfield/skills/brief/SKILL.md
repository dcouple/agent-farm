...
An HTML page for a person, rendered with the `page` house standard. Layout: [references/brief-layout.md](references/brief-layout.md). Someone who was not in the conversation should understand the problem in a minute.
...
Before calling it ready, read it as the person who will pick it up: can they say what is wrong, why it matters, how success is observed, and which decisions are still open? Set the status chip accordingly. The planner sends the brief to `socrates` when the person is ready to choose a direction.

## Banned

- An architecture or implementation presented as decided
- Acceptance criteria written as implementation steps
- Work packages, file lists, or a plan
