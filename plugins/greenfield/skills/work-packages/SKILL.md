...
5. **Escalate on evidence.** A failed check after one attempt promotes the same package to the next tier: `worker` to you. It never rewrites the plan. When the package is already yours, ask `advisor` what to do differently; it directs, it does not write code. Apply the advice, rerun the affected checks, and continue correcting actionable failures rather than failing the package after a fixed number of attempts. Record each attempt, evidence, and next hypothesis. If the same failure persists without progress, diagnose it and change approach with advisor input instead of repeating the same attempt. Follow the standing rules for genuine blockers, exhausted in-scope approaches, and explicit user limits. If an escalate-if fires or a product or design choice appears, stop that package and send it back to the planner with what failed, what you tried, and what is unknown.
6. **Verify and correct.** qa, fixes and affected-journey revalidation, then the draft pull request with `open-pr`: [references/review.md](references/review.md).
7. **Review and clean up** with the `final-review` skill. Keep correcting confirmed must-fix findings until review and required qa pass, or a standing-rule stop condition applies.
