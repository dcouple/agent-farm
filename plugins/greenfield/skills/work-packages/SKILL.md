...
One Astra Low agent owns every package and correction. Packages are checkpoints inside that one session. Never hand implementation to workers, another implementer, or an ad-hoc coding process.

1. **Intake.** Read the cover sheet, its validation criteria, and the linked approved design or brief. A legacy PLAN.md or card works as input as it stands; ignore its economy/standard routing. Treat a contained bug report or trivial task as one package. Ask about missing product scope; make routine file and algorithm choices yourself. If the source is an artifact link, read or check out its files with the destination's tools so you work from the actual cover sheet.
...
8. **Final review.** Open or update the draft PR with `open-pr` and get the Fable review through `final-review`, unless it is explicitly waived under that skill. Make the fixes yourself and ask for a focused follow-up on them. One full review covers the whole feature.
