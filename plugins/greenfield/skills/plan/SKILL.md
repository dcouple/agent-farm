...
- A short package table: stage ID, user-visible or backend outcome, dependencies, and journey numbers or check names. Stages are checkpoints for one implementer, not dispatches or file restrictions. Cover every affected client and entry path, including alternate composers and adapters.
...
Keep it brief enough to read before coding. The implementer decides the concrete files and steps. Do not duplicate the cover sheet in another implementation document.
