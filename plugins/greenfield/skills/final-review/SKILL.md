...
Review runs single lane with Fable by default. Record the mode at the top of the cover sheet with the plan metadata, and keep the PR and status metadata consistent with it.
...
4. You, the Astra implementer, fix every confirmed must-fix finding yourself; reviewers only report. Notes need no action. Re-run only the affected checks and journeys.
...
- A reviewer cannot be launched. That is a missing capability, not a pass. In a single review, fall back once: if `reviewer` cannot be launched, announce the substitute before dispatch, set the top review metadata to single lane — Astra (Fable unavailable), and run `second-reviewer` as the one reviewer. Record the fallback in the pull request. If neither launches, or a dual review is short a reviewer, stop.
