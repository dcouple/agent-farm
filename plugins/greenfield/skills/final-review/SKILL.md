---
name: final-review
description: Use when the build is finished and a draft pull request is open. Two independent reviewers once, one fix round, one follow-up, then clean up. Stops failed instead of looping.
---
...
1. Send the same material to both reviewers at the same time, against the same commit: the diff, the plan, bug report, or task it was built from, and any qa evidence. `reviewer` is a native child. `second-reviewer` runs on the other vendor's model: call the launcher named in your instructions with `--message`, and give it file paths and the commit, not pasted content.
2. Do not tell either reviewer how to review, and do not show one the other's findings.
3. Merge the two reports. The must-fix list is the union of both, with duplicates combined. You do not get to overrule a must-fix. If a finding contradicts a locked decision or the plan itself, do not fix it and do not drop it: mark it disputed and put it in front of the person.
4. Fix every must-fix finding. Notes are not work. Re-run only the affected checks and journeys.
5. Follow up once. Send each reviewer its own must-fix items and the fix diff. It confirms each item is resolved and says whether the fix introduced anything new. It reads only the fix diff.
...
- A reviewer cannot be launched. That is a missing capability, not a pass.
