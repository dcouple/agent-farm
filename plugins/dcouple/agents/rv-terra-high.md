...
model:
  ...
  reasoning: high
skills: []
description: "Hypothesis: Is a fast cheap reviewer worth two passes, given it disagrees with itself between runs? Reviewer benchmark: Terra on Codex. No skills, no subagents. Body text is byte-identical across every rv- profile so the only variable is the model. (vs Astra: 0.26x to 0.75x cost, 2x FASTER at 1.6 to 4.2 min, 2 of 5 vs 1 of 5; two runs with the same lens found different defects)"
---
