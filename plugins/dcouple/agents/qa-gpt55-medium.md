...
skills:
  - pr-test-automation
description: "Hypothesis: this model at medium is the combination already known to work for QA in daily use, so it calibrates the ladder. QA ladder, BASELINE: gpt-5.5 at medium with the pr-test-automation skill. This is the combination already known to work in daily use, so it calibrates the other four. Body is byte-identical across every qa- profile, so the model is the only variable. (RESULT 2026-09-20 night, run twice on the completed orch-luna tree: product-bug-found both times. Brought up the real stack with pnpm dev:app, logged in, drove the inbox, and found that SyncOrgGetStarted fails with a Spanner closed-transaction error at get_started.go:269, a defect the arm's own race-tested suite missed because its tests use a fake Spanner. Pilot 25 min with 18 screenshots and 7 videos; parallel rerun 10 min 18 s wall with 10 screenshots and 2 videos, 1,836,321 input tokens (90% cached) + 23,991 output = $2.46 at $5/$0.50/$30; pilot $3.77, clean port teardown, honest harness-vs-live labelling. The calibration rung works)"
---
