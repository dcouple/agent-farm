...
model:
  ...
  reasoning: low
skills: []
description: "Hypothesis: Does the frontier model at its cheapest rung, given a generic review instruction, find planted defects? Reviewer benchmark: Astra at low effort. No skills, no subagents. Body text is byte-identical across every rv- profile so the only variable is the model. (vs Astra medium: 0.8x cost at $0.93, 66s, same 1 of 5 recall with the generic instruction)"
---
