...
skills:
  - pr-test-automation
description: "Hypothesis: can the cheapest reviewer and scanner also drive a QA pass, on the harness where it works? QA ladder: DeepSeek V4.1 Flash on the Claude harness with the pr-test-automation skill. Unknown at QA; it holds review at $0.034 and scanning at $0.25 a module but caches only 68% of its input, which made it the most expensive cheap builder. Harness differs from the codex rungs; the skill drives Playwright as a script, so browser capability should not be harness-bound. (RESULT 2026-09-21: product-bug-found in 38 min, the slowest rung and the only one that found the CAUSE. gpt-5.5 and Luna reported the closed-transaction symptom; DeepSeek queried information_schema on the same database, found the get_started_ledgers table missing (migration unapplied), and wrote a Go reproducer proving readGetStartedLedger conflates NotFound-on-missing-table with NotFound-on-missing-row and continues on a dead transaction. Two further findings (undismissable error card, polling gated on an org selection a failed discovery never makes). 21 screenshots, 3 videos, 14 labelled harness states, withdrew two of its own early claims as fixture faults, tree untouched. 3,290,365 uncached + 7,657,680 cached input, 65,539 output = $0.56, a fifth of the baseline)"
---
