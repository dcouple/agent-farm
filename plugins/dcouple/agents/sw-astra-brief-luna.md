...
description: "Sub-only version of the phase-boundary planner: Astra LOW writes scope and per-phase briefs, Luna xhigh builds and tests as a detached process, Astra is resumed once by the waiter, reviews once, accepts or issues a fix brief to fx-luna-verify. Hypothesis: Astra tokens per task fall from 2.8M (Astra alone) to two short turns, the 98% shape on a subscription. (RESULT 2026-09-20: builder tree passes all six contract commands independently (Luna xhigh, 27.8M tokens, $0.68); Astra 104k brief turn + 407k review turn = 511k total, 82% under Astra alone; review found a non-verbatim refusal message the gates missed and dispatched fx-luna-verify, which fixed it in 6.6 min for $0.06 (FIXED 1 / REJECTED none); the tree was re-verified by hand after the fix. ASTRA ITSELF: one brief turn, 103,871 input + 2,010 output tokens, $0.46, which is 96% under Astra alone (2.8M, $4.42). CAVEAT, 18th retraction: the review and acceptance turns ran on gpt-daybreak-blue-latest, not Astra, because `codex exec resume` falls back to the config default; notify.sh now pins the model and the review must be re-measured)"
---
