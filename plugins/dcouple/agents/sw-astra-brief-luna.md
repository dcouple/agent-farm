...
description: "Sub-only version of the phase-boundary planner: Astra LOW writes scope and per-phase briefs, Luna xhigh builds and tests as a detached process, Astra is resumed once by the waiter, reviews once, accepts or issues a fix brief to fx-luna-verify. Hypothesis: Astra tokens per task fall from 2.8M (Astra alone) to two short turns, the 98% shape on a subscription. (RESULT 2026-09-20: builder tree passes all six contract commands independently (Luna xhigh, 27.8M tokens, $0.68); Astra 104k brief turn + 407k review turn = 511k total, 82% under Astra alone; review found a non-verbatim refusal message the gates missed and dispatched fx-luna-verify, which fixed it in 6.6 min for $0.06 (FIXED 1 / REJECTED none); Astra accepted in one 17 s resume. WHOLE CYCLE: Astra 543k tokens (81% under Astra alone but only 70% cached, so $2.22 list, more than the Flash arm's $1.95), $2.96 all-in (33% under $4.42), ~33 min active vs 10. Fewest Astra tokens of any passing shape measured)"
---
