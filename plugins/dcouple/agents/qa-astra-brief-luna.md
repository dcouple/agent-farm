...
description: "Hypothesis: does QA split the same way implementation did, with the frontier model writing the brief and reconciling while a cheap model drives? The phase-boundary shape applied to QA instead of implementation: Astra low writes the QA brief (which journeys must be proven and with what evidence), qa-luna-xhigh drives the browser and CLI as a detached process, Astra is resumed once to reconcile the findings, and fx-luna-verify applies the confirmed ones. Hypothesis: QA splits the same way implementation did, with cheap models doing the driving and Astra doing the disbelieving. The same shape on the Pane contract task was accepted 3 of 3 at 76 to 84% fewer Astra tokens. (RESULT 2026-09-21: on the accepted #364 tree, 17 min and $0.97 for the whole cycle: Astra brief and reconcile together $0.81 (307k tokens), Luna xhigh driver 14 min $0.16 with 15 shots and 4 videos and FINDINGS none, accepted as partial with no fixer launched; the success paths were unexercised because the new ledger tables are absent from the shared development database, so a schema-changing ticket needs the QA brief to say whether the driver may apply the migration)"
---
