...
description: "Hypothesis: on an open ticket, is the frontier model better spent building than briefing? Astra writes the brief for itself, implements it, runs the gates, then launches two cheap subscription lanes detached, a Luna xhigh reviewer with the time-bounded body and a gpt-5.5 QA driver, is resumed once to reconcile their findings as claims, fixes the confirmed ones itself, and accepts or runs one more review. The inverse of sw-astra-brief-luna, where Astra briefs and reviews and Luna builds ($6.70, three cycles, accepted on #364). (RESULT 2026-09-21 on #364: ACCEPTED with known verification gaps in 32 min, $9.87 all-in: Astra medium briefed and built in 21 min ($7.03), rv-luna-timebound found nothing in 9 min ($0.15), qa-gpt55-medium partial in 6 min ($1.51), Astra reconciled in 2 min; Astra thread 4.87M tokens $8.21. The fastest result on the ticket, but an Astra audit of the same diff found a HIGH the Luna reviewer had cleared (a partial manual trial extension consumed by a pending reward, with a reproducer); about 50 min and $16 with the fix. n=1; the review seat stays with Astra)"
---
