...
When Grain is available, read the installed Grain skill and use its
current create/update workflow. Update an existing comparison workspace
when supplied. Honor an explicit alternative destination; if Grain is
unavailable, save the HTML and images locally and report that it was not
saved to Grain. The HTML is self-contained:
...
- Grain is the default destination when available; respect a user-specified
