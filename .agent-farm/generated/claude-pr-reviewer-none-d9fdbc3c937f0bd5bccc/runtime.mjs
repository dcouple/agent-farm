...
export const connectionName = (name) => 'orchestra_' + name;
export function claudeConnection(value) {
    if ('url' in value)
...
export function codexConnection(value) {
...
export function files(root) {
    ...
    function visit(folder) {
        for (const entry of fs.readdirSync(folder, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
            ...
            if (entry.isSymbolicLink())
                throw new Error(`Symlinks are not supported in bundle inputs: ${p}`);
    ...
    if (fs.lstatSync(root).isSymbolicLink())
        throw new Error(`Symlink directory: ${root}`);
    visit(root);
    return result;
}
...
export function verify(bundle) {
    ...
    if (canonical(fileMap(bundle)) !== canonical(expected))
        throw new Error('Bundle integrity check failed; refusing modified bundle');
}
function link(source, destination) {
    ...
    if (current) {
        if (!current.isSymbolicLink() || fs.realpathSync(destination) !== fs.realpathSync(source))
            throw new Error(`Conflicting runtime path: ${destination}`);
    }
    else
        fs.symlinkSync(source, destination);
}
export function codexHome(bundle, route, env, home = os.homedir()) {
    ...
    for (const item of ['config.toml', 'auth.json', '.credentials.json', 'AGENTS.md', 'AGENTS.override.md', 'rules', 'plugins', 'mcp-oauth-locks'])
        link(path.join(original, item), path.join(runtime, item));
    ...
    if (fs.existsSync(path.join(original, 'skills')))
        for (const name of fs.readdirSync(path.join(original, 'skills'))) {
            if (!names.includes(name))
                link(path.join(original, 'skills', name), path.join(skills, name));
        }
    for (const name of names)
        link(path.join(selected, name), path.join(skills, name));
    env.CODEX_HOME = runtime;
    env.AGENT_FARM_NATIVE_CODEX_HOME = original;
    return runtime;
}
export function command(bundle, route, options = {}) {
    const manifest = JSON.parse(fs.readFileSync(path.join(bundle, 'manifest.json'), 'utf8'));
    const agent = manifest.nodes[route];
    if (!agent)
        throw new Error('Unknown bundled child');
    const directory = path.join(bundle, route);
    ...
    if (agent.harness === 'claude') {
        ...
        if (Object.keys(native).length)
            argv.push('--agents', JSON.stringify(native));
        if (agent.reasoning_effort)
            argv.push('--effort', agent.reasoning_effort);
        if (instructions)
        ...
        if (instructions)
            argv.push('-c', 'developer_instructions=' + JSON.stringify(instructions));
        if (agent.reasoning_effort)
            argv.push('-c', 'model_reasoning_effort=' + JSON.stringify(agent.reasoning_effort));
        for (const [alias, childRoute] of Object.entries(agent.children))
            if (manifest.nodes[childRoute].mode === 'native') {
        ...
        if (agent.speed)
            argv.push('-c', 'service_tier=' + JSON.stringify(agent.speed === 'fast' ? 'fast' : 'default'));
        for (const [key, value] of Object.entries(agent.connections))
            argv.push('-c', `mcp_servers.${connectionName(key)}=${toml(codexConnection(value))}`);
    }
    if (options.message !== undefined)
        argv.push('--', options.message);
    return { argv, env, cwd: manifest.directory };
}
export function execute(argv, cwd, environment) {
    if (process.platform === 'win32' || !process.execve)
        throw new Error('Native launch requires Node 22.15+ on macOS or Linux');
    const name = argv[0];
    ...
    if (!executable)
        throw new Error(`Missing native harness: ${name}`);
    const env = Object.fromEntries(Object.entries(environment).filter((pair) => pair[1] !== undefined));
    process.chdir(cwd);
...
export function run(bundle, route, args) {
    ...
    if (values.explain)
        console.log(JSON.stringify({ argv: launch.argv, cwd: launch.cwd, bundle }, null, 2));
    else
