...
export async function startTelemetryUI(options:QueryOptions,port=0,management?:{configRoot:string;projectDirectory:string}){
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    if(route===''||route==='app.js'){res.writeHead(200,{'Content-Type':route?'text/javascript; charset=utf-8':'text/html; charset=utf-8'});res.end(route?'('+telemetryBrowser.toString()+')();'+(profiles?'('+managementBrowser.toString()+')();':''):profiles?uiPage:telemetryPage);return;}
